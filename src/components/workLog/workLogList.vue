            ...
            exportWorkLogHandler() {
                this.$refs.exportForm.validate(valid => {
                    if (valid) {
                        ...
                        downloadWorklog(this.dateRange).then(res => {
                            const link = document.createElement("a"); // 创建a标签
                            link.href = this.backUrl + "?dateStart=" + this.dateRange.dateStart + "&dateEnd=" + this.dateRange.dateEnd;
                            link.click();
                            URL.revokeObjectURL(this.backUrl); // 释放内存
                        });
